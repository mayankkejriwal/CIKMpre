...
public class postProcessScores {
	...
	public static void precRecPoints(String sortedScores, ImportGoldStandard gold, String output)throws IOException{
		...
		for(int i=0; i<actual.size(); i++){
			...
			int index2=Integer.parseInt(index[0]);
		...
		for(int i=0; i<cumCorrect.size(); i++){
			...
			rec.add(cumCorrect.get(i)*1.0/(gold.num_dups));
		}
	...
	public static void precRecPointsReverse(String sortedScores, ImportGoldStandard gold, String output)throws IOException{
		...
		for(int i=0; i<cumCorrect.size(); i++){
			prec.add(cumCorrect.get(i)*1.0/(i+1));
			rec.add(cumCorrect.get(i)*1.0/(gold.total_pairs-gold.num_dups));
		}
	...
	public static void main(String[] args)throws IOException{
		...
		processBigTF.pruneLower(prefix+"TF_output1", prefix+"TF_output1_0.04",0.04);
		processBigTF.convertToOriginal(prefix+"TF_output1_0.04", prefix+"TF_0.04");
		RL.postProcessMultipleScoresRL(prefix+"TF_0.04", prefix+"sortedScores/");
		*/
		//RL.postProcessMultipleScoresRL(prefix+"C", prefix+"sortedScores/"); //be wary of this line; not included in sequence
		//RL.precRecPointsRL(prefix+"sortedScores/J",prefix+"goldStandard",prefix+"sortedScores/precRec_J");
			processSVM(prefix+"svmOutputIter",prefix+"goldStandard",
			prefix+"sortedScores/svmIter",prefix+"sortedScores/precRec_svmIter"); 
		/*		
		processSVM(prefix+"svmOutput",prefix+"goldStandard",
				prefix+"sortedScores/svm",prefix+"sortedScores/precRec_svm"); //call this for svm
		processSVM(prefix+"svmOutput50",prefix+"goldStandard",
				prefix+"sortedScores/svm50",prefix+"sortedScores/precRec_svm50"); //call this for svm
		processSVM(prefix+"svmOutput10",prefix+"goldStandard",
				prefix+"sortedScores/svm10",prefix+"sortedScores/precRec_svm10"); //call this for svm
	*/
	
	}
	
	/*
	 * Recall svmOutput is of format 1 2 0.99 0.01 (where 1 and 2 refer to indices from the two files)
	 ...
	 * for the score files 
	 */
	public static void processSVM(String svmOutput, String goldStandard, String outputTrunc, String outputPrecRec)throws IOException{
	...
	// /must be included
	public static void postProcessMultipleScoresRL(String input, String outputfolder)throws IOException{
		Scanner in=new Scanner(new FileReader(new File(input)));
		HashMap<String,HashMap<Double,HashSet<String>>> scores=new HashMap<String,HashMap<Double,HashSet<String>>>();
